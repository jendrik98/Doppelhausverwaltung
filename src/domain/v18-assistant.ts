...
export function decisionActionLabel(x){if(x.route==="rental"&&x.sub==="lifecycle")return"Mietkonto öffnen";if(x.route==="rental"&&x.sub==="calculation")return"Abrechnung prüfen";if(x.route==="rental"&&x.sub==="water")return"Wasser prüfen";if(x.route==="owner"&&x.sub==="cashflow")return"Zahlungen öffnen";if(x.route==="owner"&&x.sub==="reconciliation")return"Zuordnung prüfen";if(x.route==="data"&&x.sub==="documents")return"Dokumente prüfen";if(x.route==="data"&&x.sub==="positions")return"Kosten prüfen";if(x.route==="more"&&x.sub==="backup")return"Sicherung öffnen";return"Öffnen"}
