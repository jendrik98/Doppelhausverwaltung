...
function createEmptyState(){
  return {
    ...
    meta:{appVersion:"14.0.0",createdAt:new Date().toISOString(),migratedFrom:null,revision:0,lastSavedAt:null,lastBackupAt:null,lastIntegrityCheckAt:null,errorLog:[]},
...
function normalizeState(s){
  ...
  out.meta={...base.meta,...(s?.meta||{}),appVersion:"14.0.0"};
...
function runSelfTests(){
  ...
  results.push(assert("Navigation hat eindeutige Hauptbereiche",new Set(Object.keys(ROUTE_LABELS)).size===5));
  results.push(assert("Mietvertrag ist unter Vermietung eingeordnet",CHECK_INPUT_MAP["Mietvertrag fehlt"]?.route==="rental"));
  results.push(assert("Detailseiten besitzen sichtbare Elternbereiche",visibleSub("data","positions")==="costs"&&visibleSub("owner","reconciliation")==="payments"&&visibleSub("more","backup")==="protection"));
return results
}
...
function sectionTabs(items,active){
  return `<div class="section-tabs">${items.map(i=>`<button data-workspace-sub="${i.id}" class="${i.id===active?"active":""}">${esc(i.label)}</button>`).join("")}</div>`
}
...
const SUB_PARENT={
  data:{property:"object",units:"object",sources:"costs",positions:"costs",assessment:"costs"},
  rental:{calculation:"billing",workflow:"billing"},
  owner:{cashflow:"payments",reconciliation:"payments",finance:"planning",analytics:"planning"},
  more:{overview:"smart",security:"protection",backup:"protection",recovery:"protection",audit:"app",diagnostics:"app"}
};
let route="home";
let sub={...DEFAULT_SUB};
...
const CHECK_INPUT_MAP={
  ...
  "Mietvertrag fehlt":{route:"rental",sub:"lease",label:"Mietvertrag"},
...
function home(){
  ...
  <div class="hero"><div><p class="eyebrow">START</p><h2>Dein Haus auf einen Blick</h2><p class="muted">${esc(state.property.name||"Hausverwaltung")} · ${esc(periodLabel(currentPeriodYear()))}</p></div><div class="health ${quality>=85?"good":quality>=65?"mid":"low"}" title="Datenqualität">${quality}%</div></div>
  <div class="quick-actions-bar">
    <button data-home-action="document"><span class="quick-symbol">▤</span><span><strong>Dokument</strong><small>erfassen</small></span></button>
    <button data-home-action="meter"><span class="quick-symbol">◌</span><span><strong>Zähler</strong><small>ablesen</small></span></button>
    <button data-home-action="payment"><span class="quick-symbol">€</span><span><strong>Zahlung</strong><small>erfassen</small></span></button>
    <button data-home-action="billing"><span class="quick-symbol">✓</span><span><strong>Abrechnung</strong><small>prüfen</small></span></button>
  </div>
  <div class="grid cards overview-cards"><article class="card"><span>Hausrate</span><strong>${euro(repay)}</strong><small>monatlich</small></article><article class="card"><span>Kaltmiete</span><strong>${euro(rent)}</strong><small>monatlich</small></article><article class="card"><span>Hausrate nach Kaltmiete</span><strong class="${rateAfterRent>0?"negative":"positive"}">${euro(rateAfterRent)}</strong><small>ohne weitere Hauskosten</small></article><article class="card"><span>Mieter-BK aktuell</span><strong>${euro(a.tenantCosts)}</strong><small>${periodLabel(currentPeriodYear())}</small></article></div>
  <div class="card priority-card"><div class="card-head"><div><p class="eyebrow">PRIORITÄTEN</p><h3>Jetzt wichtig</h3><p class="muted">${rentNow.status!=="none"?`Mietzahlung: ${rentStatusLabel(rentNow)} · `:""}Abrechnungsprognose ${proj.confidence}%.</p></div><button class="secondary compact" id="openSmart">Alle Hinweise</button></div>${insights.length?insights.slice(0,4).map((x,i)=>`<button class="priority-item ${x.severity==="bad"?"legal-bad":x.severity==="warn"?"legal-warn":"info"}" data-smart-home="${i}"><span><strong>${esc(x.title)}</strong><small>${esc(x.detail||"")}</small></span><b>${Math.round(Number(x.confidence||0))}%</b></button>`).join(""):`<p class="legal-ok">✓ Aktuell keine offenen Prioritäten.</p>`}</div>
  <div class="grid two-up">
    <div class="card"><p class="eyebrow">VERTEILUNG</p><h3>Aktuelle Anteile</h3><div class="fact-row"><span>Wohnfläche Mieterin</span><strong>${percent(sh.area)}</strong></div><div class="fact-row"><span>Kaltwasseranteil</span><strong>${water?.valid?percent(water.share):"noch offen"}</strong></div><div class="fact-row"><span>Umlagefähige Kosten</span><strong>${euro(a.tenantCosts)}</strong></div></div>
    <div class="card"><p class="eyebrow">NÄCHSTE TERMINE</p><h3>Was ansteht</h3>${taskList().slice(0,3).map(taskHTML).join("")}</div>
  </div>
  ...
  $("openSmart").onclick=()=>go("more","smart");$("openPlanning").onclick=()=>go("owner","planning");
  document.querySelectorAll("[data-smart-home]").forEach(b=>b.onclick=()=>{const x=insights[Number(b.dataset.smartHome)];if(!x)return;go(x.route,x.sub||DEFAULT_SUB[x.route])});
...
function houseOverviewView(){
  const owner=unitByType(state,"owner"),rental=unitByType(state,"rental"),docs=state.documentsCache||[],openDocs=docs.filter(d=>documentWorkflowState(d)!=="done").length;
  $("workspaceBody").innerHTML=`<div class="grid cards overview-cards">
    ...
    <article class="card"><span>Dokumente offen</span><strong>${openDocs}</strong><small>${docs.length} insgesamt geladen</small></article>
  </div>
  <div class="card"><div class="card-head"><div><p class="eyebrow">HAUSVERWALTUNG</p><h3>Stammdaten & Belege</h3></div></div>
    ${hubAction("object","Objekt & Einheiten","Adresse, Wohnflächen, Baujahre und Nutzung")}
    ${hubAction("costs","Kosten & Bescheide","Kostenquellen, Positionen und Grundbesitzabgaben",String(state.costPositions.length))}
    ${hubAction("infrastructure","Zähler & Behälter","Wasserzähler, Ablesungen und Abfallbehälter",String(state.meters.length))}
    ${hubAction("documents","Dokumente","Inbox, OCR-Analyse und Belege",openDocs?`${openDocs} offen`:"")}
  </div>`;
...
function houseCostsHubView(){
  ...
  $("workspaceBody").innerHTML=`<div class="grid cards">
    ...
    <article class="card"><span>Abrechnungsperiode</span><strong>${esc(periodLabel(y))}</strong><small>aktiver Zeitraum</small></article>
  </div>
  <div class="card">
...
function financePlanningHubView(){
  ...
  $("workspaceBody").innerHTML=`<div class="grid cards">
    <article class="card"><span>12M-Prognose</span><strong class="${sum<0?"negative":"positive"}">${euro(sum)}</strong><small>geplanter Gesamtsaldo</small></article>
...
function appManagementHubView(){
  ...
  $("workspaceBody").innerHTML=`<div class="card">
...
function dataWorkspace(){
  const tabs=[
    ...
    {id:"object",label:"Objekt & Einheiten",icon:"◇"},
    {id:"costs",label:"Kosten & Bescheide",icon:"€"},
    {id:"infrastructure",label:"Zähler & Behälter",icon:"◌"},
    {id:"documents",label:"Dokumente",icon:"▤"}
  ];
  const active=sub.data||"overview",visible=visibleSub("data",active);
  $("app").innerHTML=workspaceHeader("HAUS","Haus","Stammdaten, Kosten, Technik und Belege des gesamten Hauses.",tabs,visible);
...
function rentalWorkspace(){
  const tabs=[
    {id:"overview",label:"Überblick",icon:"⌂"},
    {id:"lease",label:"Mietvertrag",icon:"▧"},
  ...
  sub.rental=active;
  $("app").innerHTML=workspaceHeader("VERMIETUNG","Vermietung","Mietvertrag, Verbrauch und Betriebskostenabrechnung an einem Ort.",tabs,visibleSub("rental",active));
...
function ownerWorkspace(){
  const tabs=[
    ...
    {id:"planning",label:"Planung & Analyse",icon:"↗"},
    {id:"tasks",label:"Erinnerungen",icon:"✓"}
  ];
  const active=sub.owner||"overview",visible=visibleSub("owner",active);
  $("app").innerHTML=workspaceHeader("FINANZEN","Finanzen","Hausrate, Zahlungen, Planung und eigene Termine – getrennt von der Mieterabrechnung.",tabs,visible);
...
function ownerOverview(){
  ...
  $("workspaceBody").innerHTML=`<div class="grid cards">
    ...
    <article class="card"><span>Hausrate nach Kaltmiete</span><strong>${euro(repay-rent)}</strong></article>
...
function smartCenterView(){
  ...
  const ask=q=>{const a=smartAnswer(state,q),host=$("smartAnswer");host.innerHTML=`<div class="smart-answer"><strong>${esc(a.title)}</strong><p>${esc(a.answer)}</p><button id="smartAnswerOpen" class="secondary">Passenden Bereich öffnen</button></div>`;$("smartAnswerOpen").onclick=()=>{if(a.route==="home")go("home");else{go(a.route,a.sub||DEFAULT_SUB[a.route])}}};
  ...
  document.querySelectorAll("[data-smart-insight]").forEach(b=>b.onclick=()=>{const x=insights[Number(b.dataset.smartInsight)];if(!x)return;go(x.route,x.sub||DEFAULT_SUB[x.route])});
...
function more(){
  const tabs=[
    ...
    {id:"legal",label:"Recht & Regeln",icon:"§"},
    {id:"protection",label:"Sicherheit",icon:"◇"},
    {id:"app",label:"App & Verlauf",icon:"•••"}
  ];
  const active=sub.more||"smart",visible=visibleSub("more",active);
  $("app").innerHTML=workspaceHeader("MEHR","Mehr","Assistent, Rechtsstand, Sicherheit und technische Verwaltung.",tabs,visible);
...
function moreOverview(){sub.more="smart";smartCenterView()}
...
function setupGlobal(){
  ...
  $("searchBtn").onclick=()=>{$("searchOverlay").classList.remove("hidden");$("searchInput").focus();search("")};
  $("searchClose").onclick=()=>$("searchOverlay").classList.add("hidden");
  $("searchInput").oninput=e=>search(e.target.value);
  $("quickBtn").onclick=()=>$("quickOverlay").classList.remove("hidden");
  $("quickClose").onclick=()=>$("quickOverlay").classList.add("hidden");
  document.querySelectorAll("[data-quick]").forEach(b=>b.onclick=()=>{
    const q=b.dataset.quick;$("quickOverlay").classList.add("hidden");
    ...
    else go("rental","lease")
  })
}
function search(q){
  const z=normalizeLabelText(q||""),rows=[];
  rows.push({title:state.property.name||"Objekt",detail:"Haus · Objektdaten",route:"data",sub:"object"});
  state.sources.forEach(s=>rows.push({title:s.name,detail:"Haus · Kostenquelle",route:"data",sub:"sources"}));
  state.costPositions.forEach(p=>rows.push({title:p.label,detail:`Haus · Kostenposition · ${euro(p.amount)}`,route:"data",sub:"positions"}));
  state.tasks.forEach(t=>rows.push({title:t.title,detail:`Finanzen · Erinnerung · ${t.due||""}`,route:"owner",sub:"tasks"}));
  state.payments.forEach(p=>rows.push({title:p.label,detail:`Finanzen · Zahlung · ${p.date} · ${euro(p.amount)}`,route:"owner",sub:"cashflow"}));
  state.meters.forEach(m=>rows.push({title:m.name,detail:`Haus · Zähler · ${m.number||"ohne Nummer"}`,route:"data",sub:"infrastructure"}));
  (state.documentsCache||[]).forEach(d=>rows.push({title:d.label||d.name,detail:`Haus · Dokument · ${documentWorkflowLabel(d)}`,route:"data",sub:"documents"}));
  state.leases.forEach(l=>rows.push({title:"Mietvertrag",detail:`Vermietung · ab ${l.start}`,route:"rental",sub:"lease"}));
  const f=rows.filter(r=>!z||normalizeLabelText(`${r.title} ${r.detail}`).includes(z)).slice(0,30);
  $("searchResults").innerHTML=f.length?f.map((r,i)=>`<button class="search-result" data-search="${i}"><span><strong>${esc(r.title)}</strong><small>${esc(r.detail)}</small></span><b>›</b></button>`).join(""):`<div class="empty-state"><strong>Keine Treffer</strong><p>Versuche einen anderen Begriff.</p></div>`;
  document.querySelectorAll("[data-search]").forEach(b=>b.onclick=()=>{const r=f[Number(b.dataset.search)];$("searchOverlay").classList.add("hidden");go(r.route,r.sub)})
}
